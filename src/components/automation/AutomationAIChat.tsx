...
import { chatWithWorkflowAssistant } from '@/ai/flows/workflow-assistant';
import { buildAutomation } from '@/ai/flows/build-automation';
      ...
      if (intent === 'build') {
        ...
        const result = await buildAutomation({
          ...
          context: {
            platform: 'web',
            userName,
            existingActions: currentWorkflow ? [currentWorkflow] : []
          }
        });
        ...
        if (onAutomationGenerated) {
          onAutomationGenerated(result.action);
        }
      } else if (intent === 'code') {
        // Extract language if specified
        const lower = input.toLowerCase();
        let language: 'csharp' | 'javascript' | 'python' = 'csharp';
        ...
        // General chat
        const result = await chatWithWorkflowAssistant({
          message: input,
